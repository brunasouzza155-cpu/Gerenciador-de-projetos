...
import { STATUS_META, fmtBRL } from "@/lib/theme";
...
function Painel({ mode }: { mode: "mock" | "supabase" }) {
  ...
  const periodStart = period === "today" ? today : period === "week" ? weekAgo : period === "month" ? monthAgo : "2000-01-01";
  ...
  const allLeaves         = activeProjects.flatMap((p) => projectLeaves(store.tasks, p.id));
  const overdueLeaves     = allLeaves.filter((t) => !t.done && t.dueDate && t.dueDate < today);
  const completedToday    = allLeaves.filter((t) => t.done && t.dueDate === today);
  ...
  const statusDist = (["andamento", "desenvolvimento", "aguardando", "pausado", "cancelado", "concluido"] as const).map((s) => ({
    ...
    value: activeProjects.filter((p) => p.status === s).length,
    color: STATUS_META[s].color,
  }));

  // Evolução mensal (últimos 12 meses - tarefas concluídas)
  const monthlyData = Array.from({ length: 6 }, (_, i) => {
    const d    = new Date();
    d.setMonth(d.getMonth() - (5 - i));
    const ym   = d.toISOString().slice(0, 7);
    const label = MONTHS_PT[d.getMonth()].slice(0, 3);
    const value = allLeaves.filter((t) => t.done && t.dueDate?.startsWith(ym)).length;
    return { label, value };
  });

  // Tarefas atrasadas
  const overdueByProject = activeProjects
    .map((p) => ({
      project: p,
    ...
    <main className="min-h-screen bg-kraft">
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatCard label="Projetos ativos" value={activeProjects.length} sub={`${projects.filter(p => p.archived).length} arquivados`} />
          <StatCard
